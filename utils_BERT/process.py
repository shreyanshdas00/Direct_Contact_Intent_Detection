    ...
    def train(self):
        ...
        for epoch in range(0, self.__dataset.num_epoch):
            ...
            for text_batch, intent_batch in tqdm(dataloader, ncols=50):
                bert_tokenizer = BERT()
                padded_text, [sorted_intent], seq_lens = self.__dataset.add_padding(
                ...
                if random_intent < self.__dataset.intent_forcing_rate:
                    intent_out = self.__model(
                        text_var, att_var, seq_lens, forced_intent=intent_var
                    )
                    ...
                    intent_out = self.__model(text_var, att_var, seq_lens)
            ...
            if dev_acc > best_dev_intent or dev_sent_acc > best_dev_sent:
                ...
                print('\nTest result: intent acc score: {:.6f}, semantic '
                      'accuracy score: {:.6f}.'.format(test_acc, test_sent_acc))
